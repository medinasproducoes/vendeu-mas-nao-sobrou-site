...
async function criarPagamento(request) {
  ...
  try {
    ...
    const orderNsu = `vendeu-nao-sobrou-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

    const payload = {
      handle: 'medina_pay',
      items: [{ quantity: 1, price: 4790, description: 'Vendeu, Mas Não Sobrou - Acesso Vitalício' }],
      order_nsu: orderNsu,
...
async function webhookInfinitePay(request) {
  try {
    ...
    console.log('Venda confirmada (Vendeu Mas Nao Sobrou):', data.order_nsu, data.paid_amount);
